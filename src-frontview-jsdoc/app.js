import { CmpAmount } from "./amount.js";
import { Icon } from "./icon.js";
import { CmpItem } from "./item.js";
import { CmpModal } from "./modal.js";
 ...
 * @prop {string} title
 * @prop {ModalSize} [size]
 * @prop {any} contentBody
 * @prop {any} [contentFooter]
 * @prop {VoidFunction} [onSubmit] 
 */

/** @typedef {[Function,ModalProps,function(CmpModal):void]} ModalRender */

/**
 * @typedef {Object} Budget
 * @prop {string} navn
 * @prop {BudgetItem[]} items
 * @prop {string} startmåned
 * @prop {number} startsaldo
 * @prop {number} nextid
 */

/**
 * @typedef {Object} Postering
 * @prop {number} itemid
 * @prop {Date} dato
 * @prop {string} beskriv
 * @prop {number} beløb
 ...
 * @param {number} start
 * @param {number} end
 * @param {function(number):any} f
 * @returns any[] */
function MapRange(start,end,f) {
  let rv=[];
  let ct=end-start+1; 
...
<table class="form">
  <tbody>
    ...
    <tr>
      <th>Startsaldo:</th>
      <td>${[CmpAmount,{value:budget.startsaldo,onChange:(/**@type number*/v)=>budget.startsaldo=v}]}</td>
    </tr>
  </tbody>
</table>
...
${budget.items.length>0?html`
  <p>
    Visning: <select #bind=${[Visning,(/**@type {"months"|"ledger"}*/v)=>Visning=v]} class="form-select d-inline-block w-auto">
      <option value="months">Måned-skema</option>
      <option value="ledger">Som kontoudtog</option>
    </select>

    &nbsp;&nbsp;&nbsp;Periode:
    <select #bind|number=${[AntalMåneder,(/**@type number*/v)=>AntalMåneder=v]} class="form-select d-inline-block w-auto">
...
${[CmpItem,{data:EditData,onGem:GemItem},(/**@type CmpItem*/mi)=>ModalItem=mi]}
${RenderModalNyt()}
${RenderModalGemt()}
${RenderModalLavKopi()}
${RenderModalSletSky()}`;
...
/**
 * @param {Postering[]} Posts
 * @returns any */
function RenderTableMonth(Posts) {
  let fraDato=ParseFraDato();
  ...
  let RådighedMd=[];
  /** @type number[] */
  let UltimoMd=[],UltimoBal=0;
  /** @type number[] */
  let LavesteMd=[],LavesteLav=0;
  let cm=0;
 ...
 * @param {Postering[]} Posts
 * @returns any  */
function RenderTableLedger(Posts) {
  return html`<table class="table" style="width:auto">
  <tbody>
    <tr>
      <th colspan="2"></th>
...
function RenderModalSletSky() {
  return [CmpModal,{
    ...
    (/**@type CmpModal*/m)=>ModalSletSky=m]; 
}

/** @returns ModalRender */
function RenderModalLavKopi() {
  return [CmpModal,{
    title:"Lav Kopi",
  ...
  (/**@type CmpModal*/m)=>ModalLavKopi=m];
}

/** @returns ModalRender */
function RenderModalNyt() {
  return [CmpModal,{
    title:"Nyt budget",
  ...
  (/**@type CmpModal*/m)=>ModalNyt=m];
}

/** @returns ModalRender */
function RenderModalGemt() {
  return [CmpModal,{
