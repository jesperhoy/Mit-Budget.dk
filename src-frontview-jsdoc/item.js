...
import { CmpCheckbox, CmpRadio } from "./checkbox_radio.js";
...
export class CmpItem {
  ...
  <td><select class="form-select"></select></td>
      ...
      <tr>
        ...
        <td>
          ${!this.data.variabelt?
          [CmpAmount,{value:this.data.fastbeløb,onChange:(/**@type number*/v)=>this.data.fastbeløb=v,required:true}]:
          html`<table>
            ${[1,2,3,4].map(ln=>html`<tr>
              ${[1,2,3].map(rw=>html`
                ...
                <td _style="padding:2px ${rw<3?'2rem':'0'} 2px 0">
                  ${[CmpAmount,{value:this.data.varbeløb[ln*3 + rw - 4],onChange:(/**@type number*/v)=>this.data.varbeløb[ln*3 + rw - 4]=v,validity:ln===1 && rw===1 && !this.#EtVarBeløbAnført() ? 'Anfør beløb for mindst en måned':''}]}
                </td>`)}
            </tr>`)}
          </table>`}
        </td>
      ...
      <tr>
        ...
        <td class=${this.data.harslut ? null :'pad'}>
          ${[CmpCheckbox,{checked:this.data.harslut,onInput:(/**@type boolean*/v)=>this.data.harslut=v,inline:true}]}
  ...
  render() {
    ...
    return [CmpModal,
       ...
       (/**@type CmpModal*/m)=>this.#MyModal=m]; 
  }
}
